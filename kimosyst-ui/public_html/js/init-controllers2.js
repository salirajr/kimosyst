...
// @000
function MainCtrl($http, $state, $rootScope, $log, $uibModal, $localStorage, Auth) {

    $rootScope.constant = {"reauth": {"itr": 3}};
    ...
    $rootScope.NVL = function (text, newText) {
    ...
    $rootScope.trimText = function (text, n) {
    ...
    $rootScope.islnTextOver = function (text, n) {
        if (text !== undefined && text !== null && text !== '' && text.length >= n)
            return true;
        else
            return false;
    ...
    $rootScope.doReAuthorize = function (sCBack) {

    };

    $rootScope.doLogOut = function () {
        delete $localStorage.ksys;
        $http.defaults.headers.common.Authorization = undefined;
        if ($localStorage.ksys === undefined || $localStorage.ksys.authorization === undefined) {
            $state.go('login');
        } else {
        }
    };

    $rootScope.postData = function (uri, payload, sCBack, fCBack) {

    };

    $rootScope.getData = function (uri, sCBack, fCBack) {
        $log.info("$rootScope.activeUser.requestLock=" + $rootScope.activeUser.requestLock);
        recGetData(uri, sCBack, fCBack, 1);
    ...
    function recGetData(uri, sCBack, fCBack, itr) {
        $log.info('Begin!');
        if ($localStorage.ksys === undefined || $localStorage.ksys.authorization === undefined) {
            $state.go('login');
        }
        ...
        $http.get(uri).success(function (rsp) {
            ...
            if (status == 406 && !$rootScope.activeUser.requestLock) {
                $rootScope.activeUser.requestLock = true;
                $log.error("It's Authentication failure! on " + itr);
                if (itr < $rootScope.constant.reauth.itr) {
                    ...
                    $uibModal.open({
                        ...
                        $log.info('Recalling');
                        recGetData(uri, sCBack, fCBack, itr + 1);
                    });
                } else {
                    $log.error("Violation ReAuthentication ");
                    $rootScope.activeUser.requestLock = false;
                }
            } else {
                $log.error('Err!');
...
function vWMutasiXIncomePrintCtrl($scope) {


}

//@012
function ReAuthorizationCtrl(Auth, $scope, $rootScope, $localStorage, $uibModalInstance, toaster, $http, $log, $timeout) {
        ...
        if ($scope.itr < 3) {
            Auth.doLogin($rootScope.activeUser.uName, $scope.password, $scope.actkey,
                    function () {
                        ...
                        toaster.warning("ReAuth Success");
                    },
                    function (err, status) {
                        toaster.warning("Failure at " + $scope.itr);
...
//@013
function loginCtrl(Auth, $rootScope, $state, $scope, toaster, $log) {
    $scope.isFailed = false;
    $scope.login = function () {
        Auth.doLogin($scope.username, $scope.password, $scope.actkey,
                function () {
                    toaster.success("Helo " + $scope.username);
                    $rootScope.activeUser = Auth.getAuth();
                    $state.go("vw.mutasixincome");
                },
                function () {
                    toaster.warning("Login failed for " + $scope.username);
                    $scope.isFailed = true;
                }
        );
    };
...
//@011
function rfrcXPressionCtrl($rootScope, $scope, $http, $log, toaster) {
    function onWarning(bdy) {
        toaster.warning({body: bdy});
    }
            ...
            if (rsp.body.xprEval === "00") {
                ...
                if ($scope.errMsg.xpr === '') {
                    if ($scope.data.selecteExprIdx === null) {
                        ...
                        onSuccess("Xprn successfully add");
                        $scope.resetForm();
                    } else {
                        ...
                        onSuccess("Xprn successfully update");
                    }
                }
    ...
    $scope.deleteExpr = function () {
        if ($scope.data.selecteExprIdx === null || $scope.data.selecteExprIdx === '') {
            onWarning("Please select Xprsn.");
        } else {
            $scope.lsXprsn.splice($scope.data.selecteExprIdx, 1);
            onSuccess("Successfully deleted of " + $scope.data.selecteExprIdx);
            $scope.resetForm();
    ...
    $scope.saveExprs = function () {
        $http.post("/ksyt/ccexpression/save/asrfnce?bank=" + $scope.refBank.selected.rname + "&noteType=" + $scope.refSlipType.selected.rname, JSON.stringify($scope.lsXprsn)).success(function (rsp) {
    ...
    function callCalcExprn(i) {
        ...
        if (temp === undefined || temp === null)
            temp = '';
        $http.post("/ksyt/ccexpression/evaluate/asrfnce", JSON.stringify({
            "V": $scope.data.memoEg,
            "in": temp,
...
//@010 #CUR
function rfrcXCrossCtrl($localStorage, $rootScope, $scope, $http, $log, toaster, $state, Ivkr) {
    ...
    $scope.getRef = function (isQuerying) {
        ...
        var group = "LIST_OF_BANK";
        if ($scope.refInput.selected !== undefined)
            group = $scope.refInput.selected.rname;

        var qReq = "/ksyt/referenz/?group=" + group + "&scope=SYST_ADMNSTR";
        if (isQuerying)
            qReq = "/ksyt/referenz/?group=" + group + "&scope=SYST_ADMNSTR&qname=" + $scope.data.refName;


        $rootScope.getData(qReq, function (rsp) {
            $scope.lsRef = rsp.body;
            $scope.data.checkedIndexRefernz = null;
        }, function (err) {
            toaster.success("failed load " + group);
        });
        /*$http.get(
         qReq,
         ).success(function (rsp) {
    ...
    $scope.doSave = function () {
        ...
        if ($scope.data.refName === null || $scope.data.refName === '' || $scope.refInput.selected === undefined) {
            ...
            $rootScope.postData("/ksyt/referenz/", {
                "id": $scope.data.refId,
                ...
                onSuccess("Ref [" + rsp.body.id + "] saved succesfully");
                $scope.doResetForm();
            }, function (err) {
                $log.info(err);
            });

            /*$http.post("/ksyt/referenz/", JSON.stringify({
             "id": $scope.data.refId,
             "gruppe": $scope.refInput.selected.rname,
             "rname": $scope.data.refName,
             "rvalue": $scope.data.refValue,
    ...
    $scope.doDelete = function () {
        ...
        if ($scope.data.refId === null || $scope.data.refName === '') {
            ...
            $http.delete("/ksyt/referenz/" + $scope.data.refId).success(function (rsp) {
                $log.info(rsp);
                onSuccess("Ref [" + $scope.data.refId + "] deleted succesfully");
                $scope.doResetForm();
            }).error(function (err) {
                $log.info(err);
            });
...
//@009
function vWMutasiXIncomeCtrl($rootScope, $scope, $http, $log, toaster) {
    function onSuccess(bdy) {
        toaster.success({body: bdy});
    }
    ...
    $scope.formatSVRTDate = "YYYY-MM-DD HH:mm:ss";
    $scope.formatTDate = "yyyy-MM-dd HH:mm";
    ...
    $scope.criteriaTgl = {};
    $scope.criteriaTgl.selected = {value: 'oncreation', text: 'Tgl-Input'};
    $scope.lsCriteriaTgl = [
        {value: '1', text: 'Tgl-Income'},
        {value: '2', text: 'Tgl-Mutasi'},
        {value: 'oncreation', text: 'Tgl-Input'},
        {value: '3', text: 'Tgl-Verifikasi'}
    ];
    ...
    $scope.getRefSlipType = function () {
        $http.get(
            ...
            $scope.lsBuktiTransfer = [];
            for (var i = 0; i < rsp.body.length; i++) {
                $scope.lsBuktiTransfer.push({desc: rsp.body[i].rvalue, text: rsp.body[i].rname});
            }
        }).error(function (err) {
            $log.info(err);
        });
    ...
    $scope.getRefAccount = function () {
        $http.get(
            ...
            $scope.lsRekeningSCC = [];
            for (var i = 0; i < rsp.body.length; i++) {
                $scope.lsRekeningSCC.push({text: rsp.body[i].rvalue, noRekening: rsp.body[i].rname});
            }
        }).error(function (err) {
            $log.info(err);
        });
    ...
    function createVwRqParam() {
        var qIncomeType = "";
        if ($scope.incomeType.selected !== undefined)
            qIncomeType = $scope.incomeType.selected.text;

        var qRekeningSCC = "";
        if ($scope.rekeningSCC.selected !== undefined)
            qRekeningSCC = $scope.rekeningSCC.selected.text;

        var qBank = "";
        if ($scope.bank.selected !== undefined)
            qBank = $scope.bank.selected.text;

        var qBuktiTransfer = "";
        if ($scope.buktiTransfer.selected !== undefined)
            qBuktiTransfer = $scope.buktiTransfer.selected.text;

        var dStart = $scope.data.dateFrom.format($scope.formatSVRTDate);
        var dEnd = $scope.data.dateTo.format($scope.formatSVRTDate);


        return "oncreation?start=" + dStart + "&end=" + dEnd + "&incomeType=" + qIncomeType + "&rekeningNo=" + qRekeningSCC + "&bank=" + qBank + "&buktiTransfer=" + qBuktiTransfer;
        //return "oncreation?start=2017-10-15 08:34:16.392&end=2017-11-19 13:51:26.027";
    }

    $scope.vw.lsCompliance = [];
    $scope.retrieveVW = function () {
        $http.get(
                "/ksyt/incomecmutasi/vw/" + createVwRqParam(),
                ).success(function (rsp) {
            $scope.vw.lsCompliance = rsp.body;
            onSuccess("Complied data loaded successfully");
    ...
    $scope.retrieveVWIncome = function () {
        $http.get(
            ...
            onSuccess("Complied data income loaded successfully");
    ...
    $scope.retrieveVWMutasi = function () {
        $http.get(
            ...
            $scope.vw.lsMutasi = rsp.body;
            onSuccess("Complied data mutasi loaded successfully");
        }).error(function (err) {
            $log.info(err);
        });
    };
    ...
    $scope.doQuery = function () {
        /*$log.info($scope.data.refId);
         $log.info($scope.incomeType);
         $log.info($scope.criteriaTgl.selected);
         $log.info($scope.data.dateFrom.format('YYYY-MM-DD hh:mm'));
         $log.info($scope.data.dateTo.format('YYYY-MM-DD hh:mm'));
         $log.info($scope.bank.selected);
         $log.info($scope.buktiTransfer.selected);*/
        $scope.retrieveVW();
    };
    ...
    $scope.parseMutasiStage = function (stage) {
        switch (stage) {
            case 9.9:
    ...
    $scope.parseIncomeStage = function (stage) {
        switch (stage) {
            case 9.9:
                return "VERIFIED";
            case 0 :
    ...
    $scope.defaults = {};
    $scope.defaults.n = 30;
    $scope.trimText = function (text, n) {
        if (text !== undefined && text !== null && text !== '' && text.length >= n)
            return text.substring(0, n - 3);
        else
            return text;
    ...
    $scope.isTrimedText = function (text, n) {
        if (text !== undefined && text !== null && text !== '' && text.length >= n)
            return true;
        else
            return false;
    ...
    $scope.NVL = function (text, newText) {
        if (text !== undefined && text !== null && text !== '')
            return text;
        else
            return newText;
...
//@008
function mutasiXCrossCtrl($rootScope, $scope, $http, $log, toaster) {
    ...
    $scope.uploadFile = function (files) {
        ...
        /*var fd = new FormData();
         ...
         fd.append("now", $rootScope.getNow());*/
        $http.post("/ksyt/xcross/upload?fileName=" + $scope.data.submission.fileName, files[0], {
            transformRequest: angular.identity, headers: {'Content-Type': undefined}
        }).success(function (rsp) {
            $scope.temp.lmutasi = rsp.body;
            onSuccess("Successfully upload: " + $scope.data.submission.fileName + ", " + $scope.temp.lmutasi.content.length + " rows");
        }).error(function (err) {
            $log.info(err);
        });
    };
    ...
    function saveMutasiAt(i, sId) {
        ...
        $http.post("/ksyt/mutasi", JSON.stringify({
            "sbssnId": sId,
            ...
            //$log.info($scope.temp.lmutasi.content.length + " " + i);
            if ($scope.state.onSavingProcess && $scope.temp.lmutasi.content.length - 1 <= $scope.state.itrSaving) {
                onWarning("Processing of '" + $scope.data.submission.fileName + "' end!");
                $scope.state.onSavingProcess = false;
                $scope.state.saved = true;
            }
        }).error(function (err) {
            angular.extend($scope.temp.lmutasi.content[i], {status: 'FAILED'});
            //$log.info($scope.temp.lmutasi.content.length + " " + i);
            if ($scope.state.onSavingProcess && $scope.temp.lmutasi.content.length - 1 <= $scope.state.itrSaving) {
                onWarning("Processing of '" + $scope.data.submission.fileName + "' end!");
                $scope.state.onSavingProcess = false;
    ...
    function saveSubmision() {
        $http.post("/ksyt/mutasisubmission/", JSON.stringify({
            ...
            onSuccess("Submission of " + $scope.data.submission.fileName + " success with id=" + $scope.data.submission.id);
...
//@007
function inputPemasukanCtrl($rootScope, $scope, $http, $log, toaster, SweetAlert) {
    ...
    function postIXDetail(i, payload) {
        $http.post("/ksyt/incomexdetail/", payload).success(function (rsp) {
            $log.info(rsp);
            $scope.lsHookDetail[i].incomexdetailId = rsp.body.incomexdetailId;
            onSuccess("Detail data[" + $scope.lsHookDetail[i].incomexdetailId + "] inserted succesfully");
        }).error(function (err) {
            $log.info(err);
        });
    }
    ...
    function keepIncome() {
        if ($scope.data.incomeId !== undefined && $scope.data.incomeId !== null && $scope.data.incomeId !== '') {
            onWarning("$scope.data.incomeId=" + $scope.data.incomeId);
        } else {
            onSuccess("$scope.data.incomeId=" + $scope.data.incomeId);
        }
        $log.info($scope.data.date);
        ...
        var payloadIncome = JSON.stringify({
    ...
    $scope.calculateWeight = function () {
        ...
        onSuccess("Calculate weight starting");
    ...
    function callWCalc(i, incomeId, mutasiId) {
        $http.get(
            ...
            if (nCalculateWeight === 0) {
                ...
                onWarning("Calculate weight end");
            }
        }).error(function (err) {
            $log.info(err);
        });
    ...
    $scope.storePair = function () {
        alert($scope.data.selectedIndexCMutasi);
        ...
        if ($scope.data.selectedIndexCMutasi !== -1 && $scope.lsCMutasi[$scope.data.selectedIndexCMutasi].id !== undefined) {
            ...
            onWarning("No mutasi data selected!");
        }
    };
    function comply(i) {
        if ($scope.data.incomeCMutasiId === null) {
            $http.post("/ksyt/incomecmutasi/comply", JSON.stringify({
                ...
                onSuccess("$scope.data.incomeCMutasiId=" + $scope.data.incomeCMutasiId);
                complyImg();
            }).error(function (err) {
                $log.info(err);
            });
    ...
    function complyImg() {
        if ($scope.data.incomeCMutasiId !== null) {
            $http.post("/ksyt/incomecmutasi/complied/img/" + $scope.data.incomeCMutasiId).success(function (rsp) {
                $scope.data.incomeCMutasiFSL = "/ksyt/incomecmutasi/img/" + $scope.data.incomeCMutasiId;
                $log.info(rsp);
                onSuccess("Compliencd image has been created, " + $scope.data.incomeCMutasiFSL);
            ...
            onWarning("Compliencd image can not be created! please save complience data first!");
        }
    }
    ...
    function clusterLite() {
        $http.get(
            ...
            onSuccess("Cluster Lite telah diload, kode:1_0");
        }).error(function (err) {
            $log.info(err);
        });
    }
    ...
    function clusterRete() {
        $http.get(
            ...
            $scope.lsCMutasi = rsp.body;
            onSuccess("Cluster Rete telah diload, kode:2_1");
        }).error(function (err) {
            $log.info(err);
        });
    }
    ...
    function clusterEnti() {
        $http.get(
            ...
            $scope.lsCMutasi = rsp.body;
            onSuccess("Cluster Enti telah diload, kode:2_1");
        }).error(function (err) {
            $log.info(err);
        });
    }
...
function mutasiCtrl($rootScope, $scope, $http, $log) {
    ...
    $http.get("/ksyt/submission/assign?file=E:\Branches\kampf\kimosyst\forms\sub2keu-pembangunan-140517.xls")
            .then(function (response) {
                $scope.lsubmission = response.data.body;
            });
}
...
angular
        ...
        .controller('mutasiCtrl', mutasiCtrl)
        .controller('mutasiXCrossCtrl', mutasiXCrossCtrl)
        .controller('inputPemasukanCtrl', inputPemasukanCtrl)
        .controller('vWMutasiXIncomeCtrl', vWMutasiXIncomeCtrl)
        .controller('rfrcXCrossCtrl', rfrcXCrossCtrl)
        .controller('rfrcXPressionCtrl', rfrcXPressionCtrl)
        .controller('loginCtrl', loginCtrl)
        .controller('ReAuthorizationCtrl', ReAuthorizationCtrl)
        .controller('vWMutasiXIncomePrintCtrl', vWMutasiXIncomePrintCtrl);
