/* global moment */


// @000
function MainCtrl(CONST) {

    this.formatSVRTDate = CONST.formatSVRTDate;
    this.formatTDate = CONST.formatTDate;

    this.getNow = function () {
        return moment(new Date()).format(CONST.formatSVRTDate);
    };
    this.getNow = function () {
        return moment(new Date()).format(CONST.formatSVRDate);
    };
    this.NVL = function (text, newText) {
        if (text === undefined || text === null || text === '')
            return newText;
        else
            return text;
    };
    this.trimText = function (text, n) {
        if (text !== undefined && text !== null && text !== '' && text.length >= n)
            return text.substring(0, n - 3);
        else
            return text;
    };
    this.islnTextOver = function (text, n) {
        if (text !== undefined && text !== null && text !== '' && text.length >= n)
            return true;
        else
            return false;
    };

    this.trimNText = function (text, n) {
        var result = "";
        var cText = text.split(" ");
        for (var i = 0; i < cText.length; i++) {
            if (cText[i].length > n) {
                var spCText = Math.ceil(cText[i].length / n);
                for (var j = 0; j < spCText - 1; j++) {
                    if (j * n > cText[i].length) {
                        result += cText[i].substring(j * n, cText[i].length) + " ";
                    } else {
                        result += cText[i].substring(j * n, (j + 1) * n) + " ";
                    }
                }
            } else {
                result += cText[i] + " ";
            }
        }
        return result;
    };

    this.parseComplianceStage = function (stage) {
        if (1 <= stage && stage < 2)
            return "UnCOMPLIed";
        else if (9 <= stage && stage < 10)
            return "PAIRed";

    };
    this.parseMutasiStage = function (stage) {
        if (1 <= stage && stage < 2)
            return "CORRUPT";
        else if (2 <= stage && stage < 3)
            return "DEL";
        else if (5 <= stage && stage < 6)
            return "ReqIDTF";
        else if (6 <= stage && stage < 7)
            return "UnPAIRed";
        else if (9 <= stage && stage < 10)
            return "IDTFed";
    };
    this.parseIncomeStage = function (stage) {
        if (1 <= stage && stage < 2)
            return "CORRUPT";
        else if (2 <= stage && stage < 3)
            return "DEL";
        else if (5 <= stage && stage < 6)
            return "ReqVRFY";
        else if (6 <= stage && stage < 7)
            return "UnPAIRed";
        else if (9 <= stage && stage < 10)
            return "VRFYed";
    };
}
;
function vwIncomeFindAllCtrl(CONST, Ivkr, Dalg, $scope, toaster, $log) {

    $scope.data = {};

    $scope.criteriaTgl = {};
    $scope.lsCriteriaTgl = [];
    $scope.getRefCriteriaTgl = function () {
        Ivkr.get(
                "/ksyt/referenz/?group=MUTASI_QUERY_TYPE&scope=SYST_OWNER",
                function (rsp) {
                    $scope.lsCriteriaTgl = rsp.body;
                    //$scope.lsCriteriaTgl.push({"id": -1, "gruppe": "MUTASI_QUERY_TYPE", "rname": "[ANY-TGL]", "rvalue": "", "scope": "SYST_OWNER"});
                    $scope.criteriaTgl.selected = rsp.body[0];
                    $scope.getRefBank();
                },
                function (err) {
                    $log.error(err);
                }

        );
    };
    $scope.getRefCriteriaTgl();
    $scope.bank = {};
    $scope.lsBank = [];
    $scope.getRefBank = function () {
        Ivkr.get(
                "/ksyt/referenz/?group=LIST_OF_BANK&scope=SYST_ADMNSTR",
                function (rsp) {
                    $scope.lsBank = rsp.body;
                    $scope.lsBank.push({"id": -1, "gruppe": "LIST_OF_BANK", "rname": "[ANY-BANK]", "rvalue": "", "scope": "SYST_ADMNSTR"});
                    $scope.bank.selected = rsp.body[0];
                    $scope.getRefIncomeType();
                },
                function (err) {
                    $log.error(err);
                }

        );
    };

    $scope.incomeType = {};
    $scope.lsIncomeType = [];
    $scope.getRefIncomeType = function () {
        Ivkr.get(
                "/ksyt/referenz/?group=LIST_OF_INCOME_TYPE&scope=SYST_ADMNSTR",
                function (rsp) {
                    $scope.lsIncomeType = rsp.body;
                    $scope.lsIncomeType.push({"id": -1, "gruppe": "LIST_OF_INCOME_TYPE", "rname": "[ANY-INCOME-TYPE]", "rvalue": "", "scope": "SYST_ADMNSTR"});
                    $scope.incomeType.selected = rsp.body[0];
                    $scope.getRefSlipType();
                },
                function (err) {
                    $log.error(err);
                }

        );
    };

    $scope.slipType = {};
    $scope.lsSlipType = [];
    $scope.getRefSlipType = function () {

        Ivkr.get(
                "/ksyt/referenz/?group=LIST_OF_SLIP_TYPE&scope=SYST_ADMNSTR",
                function (rsp) {
                    $scope.lsSlipType = rsp.body;
                    $scope.lsSlipType.push({"id": -1, "gruppe": "LIST_OF_SLIP_TYPE", "rname": "[ANY-SLIP-TYPE]", "rvalue": "", "scope": "SYST_ADMNSTR"});
                    $scope.slipType.selected = rsp.body[0];
                    $scope.getRefAccount();
                },
                function (err) {
                    $log.error(err);
                }
        );
    };

    $scope.account = {};
    $scope.lsAccount = [];
    $scope.getRefAccount = function () {
        Ivkr.get(
                "/ksyt/referenz/?group=LIST_OF_REKENING_SCC&scope=SYST_ADMNSTR",
                function (rsp) {
                    $scope.lsAccount = rsp.body;
                    $scope.lsAccount.push({"id": -1, "gruppe": "LIST_OF_REKENING_SCC", "rname": "", "rvalue": "[ANY-REKENING-SCC]", "scope": "SYST_ADMNSTR"});
                    $scope.account.selected = rsp.body[0];

                    /* EXECUTEION OF QUERY */
                    $scope.retrieveVWIncomeFindAll();
                },
                function (err) {
                    $log.error(err);
                }
        );
    };

    $scope.data.dateFrom = moment().startOf('month');
    $scope.data.dateTo = new moment();
    function createVwRqParam() {

        var qIncomeType = "";
        if ($scope.incomeType.selected !== undefined && $scope.incomeType.selected.rname !== '[ANY-INCOME-TYPE]')
            qIncomeType = $scope.incomeType.selected.rname;
        var qAccount = "";
        if ($scope.account.selected !== undefined && $scope.account.selected.rname !== '[ANY-INCOME-TYPE]')
            qAccount = $scope.account.selected.rname;
        var qBank = "";
        if ($scope.bank.selected !== undefined && $scope.bank.selected.rname !== '[ANY-BANK]')
            qBank = $scope.bank.selected.rname;
        var qSlipType = "";
        if ($scope.slipType.selected !== undefined && $scope.slipType.selected.rname !== '[ANY-SLIP-TYPE]')
            qSlipType = $scope.slipType.selected.rname;

        var dStart = $scope.data.dateFrom.format(CONST.formatSVRTDate);
        var dEnd = $scope.data.dateTo.format(CONST.formatSVRTDate);

        var rqParam = "oncreation?gtStage=0&start=" + dStart + "&end=" + dEnd + "&incomeType=" + qIncomeType + "&rekeningNo=" + qAccount + "&bank=" + qBank + "&buktiTransfer=" + qSlipType;
        $log.info("createVwRqParam=" + rqParam);
        return rqParam;
        //return "oncreation?start=2018-01-01 00:00:00&end=2019-01-12 23:19:42&incomeType=&rekeningNo=&bank=&buktiTransfer=&gtStage=0";
    }

    $scope.resetVwRqParam = function () {
        Dalg.confirm("You are reseting search-query?", function (out) {
            $scope.criteriaTgl.selected = $scope.lsCriteriaTgl[$scope.lsCriteriaTgl.length - 1];
            $scope.bank.selected = $scope.lsBank[$scope.lsBank.length - 1];
            $scope.incomeType.selected = $scope.lsIncomeType[$scope.lsIncomeType.length - 1];
            $scope.slipType.selected = $scope.lsSlipType[$scope.lsSlipType.length - 1];
            $scope.account.selected = $scope.lsAccount[$scope.lsAccount.length - 1];
            $scope.data.refId = "";
            $scope.data.dateFrom = moment().startOf('month');
            $scope.data.dateTo = new moment();
            $scope.retrieveVWIncomeFindAll();
        });

    };



    $scope.vw = {};
    $scope.vw.lsIncome = [];
    $scope.retrieveVWIncomeFindAll = function () {
        Ivkr.get(
                "/ksyt/income/vw/" + createVwRqParam(),
                function (rsp) {
                    $scope.vw.lsIncome = rsp.body;
                    toaster.success("Complied data income loaded successfully");
                    $log.info(rsp);
                }, function (err) {
            $log.info(err);
        }
        );
    };



}

//@009
function vwMutasiIncomeCompliance(CONST, Ivkr, Dalg, $scope, toaster, $log) {

    $scope.data = {};

    $scope.criteriaTgl = {};
    $scope.lsCriteriaTgl = [];
    $scope.getRefCriteriaTgl = function () {
        Ivkr.get(
                "/ksyt/referenz/?group=MUTASI_QUERY_TYPE&scope=SYST_OWNER",
                function (rsp) {
                    $scope.lsCriteriaTgl = rsp.body;
                    //$scope.lsCriteriaTgl.push({"id": -1, "gruppe": "MUTASI_QUERY_TYPE", "rname": "[ANY-TGL]", "rvalue": "", "scope": "SYST_OWNER"});
                    $scope.criteriaTgl.selected = rsp.body[0];
                    $scope.getRefBank();
                },
                function (err) {
                    $log.error(err);
                }

        );
    };
    $scope.getRefCriteriaTgl();
    $scope.bank = {};
    $scope.lsBank = [];
    $scope.getRefBank = function () {
        Ivkr.get(
                "/ksyt/referenz/?group=LIST_OF_BANK&scope=SYST_ADMNSTR",
                function (rsp) {
                    $scope.lsBank = rsp.body;
                    $scope.lsBank.push({"id": -1, "gruppe": "LIST_OF_BANK", "rname": "[ANY-BANK]", "rvalue": "", "scope": "SYST_ADMNSTR"});
                    $scope.bank.selected = rsp.body[0];
                    $scope.getRefIncomeType();
                },
                function (err) {
                    $log.error(err);
                }

        );
    };

    $scope.incomeType = {};
    $scope.lsIncomeType = [];
    $scope.getRefIncomeType = function () {
        Ivkr.get(
                "/ksyt/referenz/?group=LIST_OF_INCOME_TYPE&scope=SYST_ADMNSTR",
                function (rsp) {
                    $scope.lsIncomeType = rsp.body;
                    $scope.lsIncomeType.push({"id": -1, "gruppe": "LIST_OF_INCOME_TYPE", "rname": "[ANY-INCOME-TYPE]", "rvalue": "", "scope": "SYST_ADMNSTR"});
                    $scope.incomeType.selected = rsp.body[0];
                    $scope.getRefSlipType();
                },
                function (err) {
                    $log.error(err);
                }

        );
    };

    $scope.slipType = {};
    $scope.lsSlipType = [];
    $scope.getRefSlipType = function () {

        Ivkr.get(
                "/ksyt/referenz/?group=LIST_OF_SLIP_TYPE&scope=SYST_ADMNSTR",
                function (rsp) {
                    $scope.lsSlipType = rsp.body;
                    $scope.lsSlipType.push({"id": -1, "gruppe": "LIST_OF_SLIP_TYPE", "rname": "[ANY-SLIP-TYPE]", "rvalue": "", "scope": "SYST_ADMNSTR"});
                    $scope.slipType.selected = rsp.body[0];
                    $scope.getRefAccount();
                },
                function (err) {
                    $log.error(err);
                }
        );
    };

    $scope.account = {};
    $scope.lsAccount = [];
    $scope.getRefAccount = function () {
        Ivkr.get(
                "/ksyt/referenz/?group=LIST_OF_REKENING_SCC&scope=SYST_ADMNSTR",
                function (rsp) {
                    $scope.lsAccount = rsp.body;
                    $scope.lsAccount.push({"id": -1, "gruppe": "LIST_OF_REKENING_SCC", "rname": "", "rvalue": "[ANY-REKENING-SCC]", "scope": "SYST_ADMNSTR"});
                    $scope.account.selected = rsp.body[0];

                    /* EXECUTEION OF QUERY */
                    $scope.retrieveVWMutasiCompliance();
                },
                function (err) {
                    $log.error(err);
                }
        );
    };

    $scope.data.dateFrom = moment().startOf('month');
    $scope.data.dateTo = new moment();
    function createVwRqParam() {

        var qIncomeType = "";
        if ($scope.incomeType.selected !== undefined && $scope.incomeType.selected.rname !== '[ANY-INCOME-TYPE]')
            qIncomeType = $scope.incomeType.selected.rname;
        var qAccount = "";
        if ($scope.account.selected !== undefined && $scope.account.selected.rname !== '[ANY-INCOME-TYPE]')
            qAccount = $scope.account.selected.rname;
        var qBank = "";
        if ($scope.bank.selected !== undefined && $scope.bank.selected.rname !== '[ANY-BANK]')
            qBank = $scope.bank.selected.rname;
        var qSlipType = "";
        if ($scope.slipType.selected !== undefined && $scope.slipType.selected.rname !== '[ANY-SLIP-TYPE]')
            qSlipType = $scope.slipType.selected.rname;

        var dStart = $scope.data.dateFrom.format(CONST.formatSVRTDate);
        var dEnd = $scope.data.dateTo.format(CONST.formatSVRTDate);

        var rqParam = "oncreation?start=" + dStart + "&end=" + dEnd + "&incomeType=" + qIncomeType + "&rekeningNo=" + qAccount + "&bank=" + qBank + "&buktiTransfer=" + qSlipType;
        $log.info("createVwRqParam=" + rqParam);
        return rqParam;
        //return "oncreation?start=2018-01-01 00:00:00&end=2019-01-12 23:19:42&incomeType=&rekeningNo=&bank=&buktiTransfer=&gtStage=0";
    }

    $scope.resetVwRqParam = function () {
        Dalg.confirm("You are reseting search-query?", function (out) {
            $scope.criteriaTgl.selected = $scope.lsCriteriaTgl[$scope.lsCriteriaTgl.length - 1];
            $scope.bank.selected = $scope.lsBank[$scope.lsBank.length - 1];
            $scope.incomeType.selected = $scope.lsIncomeType[$scope.lsIncomeType.length - 1];
            $scope.slipType.selected = $scope.lsSlipType[$scope.lsSlipType.length - 1];
            $scope.account.selected = $scope.lsAccount[$scope.lsAccount.length - 1];
            $scope.data.refId = "";
            $scope.data.dateFrom = moment().startOf('month');
            $scope.data.dateTo = new moment();
            $scope.retrieveVWMutasiCompliance();
        });

    };

    $scope.vw = {};

    $scope.vw.mutasiCompliance = [];
    $scope.retrieveVWMutasiCompliance = function () {
        Ivkr.get(
                "/ksyt/incomecmutasi/vw/" + createVwRqParam(),
                function (rsp) {
                    $scope.vw.mutasiCompliance = rsp.body;
                    toaster.success("Complied data Mutasi-Compliance loaded successfully");
                    $log.info(rsp);
                    $scope.retrieveVWMutasi();
                }, function (err) {
            $log.info(err);
        }
        );
    };


    $scope.vw.mutasi = [];
    $scope.retrieveVWMutasi = function () {
        Ivkr.get(
                "/ksyt/mutasi/vw/" + createVwRqParam(),
                function (rsp) {
                    $scope.vw.mutasi = rsp.body;
                    toaster.success("Complied data Mutasi loaded successfully");
                    $log.info(rsp);
                    $scope.retrieveVWIncome();
                }, function (err) {
            $log.info(err);
        }
        );
    };

    $scope.vw.income = [];
    $scope.retrieveVWIncome = function () {
        Ivkr.get(
                "/ksyt/income/vw/" + createVwRqParam() + "&gtStage=5.0&ltStage=9.0",
                function (rsp) {
                    $scope.vw.income = rsp.body;
                    toaster.success("Complied data income loaded successfully");
                    $log.info(rsp);
                }, function (err) {
            $log.info(err);
        }
        );
    };


}

function vwMutasiSubmission(CONST, Ivkr, Dalg, $scope, $location, toaster, $log) {
    $scope.data = {};
    $scope.data.dateFrom = moment().startOf('month');
    $scope.data.dateTo = new moment();

    $scope.data.locationSearchId = $location.search().id;
    $scope.data.submissionId = $location.search().id;

    $scope.selectMutasiSubmission = function (id) {
        $log.info(id);
        $scope.data.refId = id;
        $scope.data.isChecked = true;
    };

    $scope.cancelCheck = function () {
        $scope.data.isChecked = false;
        $scope.data.refId = '';
    };

    $scope.voidMutasi = function () {
        toaster.warning("Mutasi void");
        $scope.data.isChecked = false;
        $scope.data.refId = '';
    };

    $scope.bank = {};
    $scope.lsBank = [];
    $scope.getRefBank = function () {
        Ivkr.get(
                "/ksyt/referenz/?group=LIST_OF_BANK&scope=SYST_ADMNSTR",
                function (rsp) {
                    $scope.lsBank = rsp.body;
                    $scope.lsBank.push({"id": -1, "gruppe": "LIST_OF_BANK", "rname": "[ANY-BANK]", "rvalue": "", "scope": "SYST_ADMNSTR"});
                    $scope.bank.selected = rsp.body[0];
                    $scope.getRefAccount();
                },
                function (err) {
                    $log.error(err);
                }

        );
    };
    $scope.getRefBank();

    $scope.account = {};
    $scope.lsAccount = [];
    $scope.getRefAccount = function () {
        Ivkr.get(
                "/ksyt/referenz/?group=LIST_OF_REKENING_SCC&scope=SYST_ADMNSTR",
                function (rsp) {
                    $scope.lsAccount = rsp.body;
                    $scope.lsAccount.push({"id": -1, "gruppe": "LIST_OF_REKENING_SCC", "rname": "", "rvalue": "[ANY-REKENING-SCC]", "scope": "SYST_ADMNSTR"});
                    $scope.account.selected = rsp.body[0];

                    /* EXECUTEION OF QUERY */
                    $scope.retrieveVWMutasi();
                },
                function (err) {
                    $log.error(err);
                }
        );
    };

    function createVwRqParam() {
        var dStart = $scope.data.dateFrom.format(CONST.formatSVRTDate);
        var dEnd = $scope.data.dateTo.format(CONST.formatSVRTDate);

        var rqParam = "oncreation?gtStage=0&start=" + dStart + "&end=" + dEnd + "&submissionId=" + $scope.data.submissionId;
        $log.info("createVwRqParam=" + rqParam);
        return rqParam;
        //return "oncreation?start=2018-01-01 00:00:00&end=2019-01-12 23:19:42&incomeType=&rekeningNo=&bank=&buktiTransfer=&gtStage=0";
    }

    $scope.resetVwRqParam = function () {
        Dalg.confirm("You are reseting search-query?", function (out) {
            $scope.bank.selected = $scope.lsBank[$scope.lsBank.length - 1];
            $scope.account.selected = $scope.lsAccount[$scope.lsAccount.length - 1];
            $scope.data.refId = "";
            $scope.data.dateFrom = moment().startOf('month');
            $scope.data.dateTo = new moment();
            $scope.data.isChecked = false;
            $scope.retrieveVWMutasi();
        });

    };

    $scope.deleteSubmissioin = function () {
        Dalg.delete("Are you sure want to delete submission [" + $scope.data.submissionId + "] ?",
                "If you continue this action, current inputed data will erased from system, and you won't be able to retrieve it in future",
                function (out) {

                });
    };

    $scope.vw = {};

    $scope.vw.mutasi = [];
    $scope.retrieveVWMutasi = function () {
        Ivkr.get(
                "/ksyt/mutasi/vw/" + createVwRqParam(),
                function (rsp) {
                    $scope.vw.mutasi = rsp.body;
                    toaster.success("Complied data Mutasi loaded successfully");
                    $log.info(rsp);
                }, function (err) {
            $log.info(err);
        }
        );
    };

    $scope.deleteMutasi = function () {
        Dalg.delete("Are you sure want to delete mutasi [" + $scope.data.refId + "] ?",
                "This mutasi will not appear in compliance income.",
                function (out) {
                    $log.info(out);
                    Ivkr.delete(
                            "/ksyt/mutasi/" + $scope.data.refId + "?reason=" + out.reason,
                            function (rsp) {
                                toaster.success(rsp.body.content.info);
                                $log.info(rsp);
                            }, function (err) {
                        $log.info(err);
                    }
                    );
                });
    };
}

//@019
function vwMutasiFindSubmission(CONST, Ivkr, Dalg, $scope, $location, toaster, $log) {

    $scope.data = {};
    $scope.data.dateFrom = moment().startOf('month');
    $scope.data.dateTo = new moment();

    $scope.goToDetail = function () {
        $location.path('/vwmutasi/submission').search('id=' + $scope.data.checkedId);
    };

    $scope.data.checkedId = -1;
    $scope.selectMutasiSubmission = function (id) {
        $scope.data.checkedId = id;
    };

    function createVwRqParam() {
        var dStart = $scope.data.dateFrom.format(CONST.formatSVRTDate);
        var dEnd = $scope.data.dateTo.format(CONST.formatSVRTDate);

        var rqParam = "oncreation?gtStage=0&start=" + dStart + "&end=" + dEnd;
        $log.info("createVwRqParam=" + rqParam);
        return rqParam;
        //return "oncreation?start=2018-01-01 00:00:00&end=2019-01-12 23:19:42&incomeType=&rekeningNo=&bank=&buktiTransfer=&gtStage=0";
    }

    $scope.resetVwRqParam = function () {
        Dalg.confirm("You are reseting search-query?", function (out) {
            $scope.criteriaTgl.selected = $scope.lsCriteriaTgl[$scope.lsCriteriaTgl.length - 1];
            $scope.bank.selected = $scope.lsBank[$scope.lsBank.length - 1];
            $scope.account.selected = $scope.lsAccount[$scope.lsAccount.length - 1];
            $scope.data.refId = "";
            $scope.data.dateFrom = moment().startOf('month');
            $scope.data.dateTo = new moment();
            $scope.retrieveVWMutasiSubmission();
        });

    };

    $scope.vw = {};

    $scope.vw.mutasiSubmission = [];
    $scope.retrieveVWMutasiSubmission = function () {
        Ivkr.get(
                "/ksyt/mutasisubmission/vw/" + createVwRqParam(),
                function (rsp) {
                    $scope.vw.mutasiSubmission = rsp.body;
                    toaster.success("Complied data Mutasi-Submission loaded successfully");
                    $log.info(rsp);
                }, function (err) {
            $log.info(err);
        }
        );
    };
    $scope.retrieveVWMutasiSubmission();

}


//@012
function ReAuthorizationCtrl(Auth, $scope, $rootScope, $uibModalInstance, toaster, $log, $timeout) {
    $log.info("ReAuthorizationCtrl land: " + new Date());
    $scope.itr = 1;
    $scope.actionLock = false;
    $scope.ok = function () {
        $scope.actionLock = true;
        if ($scope.itr < 3) {
            Auth.doLogin($rootScope.activeUser.uName, $scope.password, $scope.actkey,
                    function () {
                        $uibModalInstance.close(1);
                        toaster.success("ReAuth Success");
                        $scope.actionLock = false;
                    },
                    function (err, status) {
                        toaster.warning("Auth failed, itration: " + $scope.itr);
                        $scope.itr = $scope.itr + 1;
                        $scope.actionLock = false;
                    },
                    );
        } else {
            toaster.warning("3 times failure, You will be direct to login page");
            $timeout(function () {
                $uibModalInstance.close(1);
                $rootScope.logout();
            }, 5000);
        }


    };
    $scope.cancel = function () {
        $scope.actionLock = true;
        Auth.doLogout(function () {
            toaster.warning("You will be direct to login page");
        });
        $timeout(function () {
            $uibModalInstance.close(0);
        }, 5000);
    };
}
;

function ConfirmationDialogCtrl($uibModalInstance, $scope, $log, input) {
    $scope.in = input;
    $scope.confirmed = function () {
        $uibModalInstance.close($scope.out);
    };

    $scope.dismised = function () {
        $uibModalInstance.dismiss('cancel');
    };
}
;

//@013
function loginCtrl(Auth, $rootScope, $state, $scope, $log) {
    $scope.isFailed = false;
    $scope.statusMsg = "NONE";
    $scope.isHitted = false;
    $scope.itr = 0;
    $scope.login = function () {
        if ($scope.username && $scope.password && $scope.actkey) {
            $scope.isHitted = true;
            Auth.doLogin($scope.username, $scope.password, $scope.actkey,
                    function () {
                        $scope.statusMsg = "Helo " + $scope.username + ", you'll be directing to main page!";
                        $rootScope.activeUser = Auth.getAuth();
                        $state.go("mutasi.inputraws");
                        $scope.isFailed = false;
                    },
                    function () {
                        $scope.statusMsg = "Invalid authorization, contact your administrators";
                        $scope.isFailed = true;
                        $scope.isHitted = false;
                    }
            );
            $scope.itr = $scope.itr + 1;
        } else {
            $scope.statusMsg = "Incomplete authorization data!)";
            $scope.isFailed = true;
        }

    };
}

//@011
function rfrcXPressionCtrl(Ivkr, $scope, $http, $log, toaster) {

    $scope.temp = {};
    $scope.state = {};
    $scope.reference = {};
    $scope.data = {};
    $scope.errMsg = {};
    $scope.errMsg.xpr = "";
    $scope.lsRefBank = [];
    $scope.refBank = {};
    $scope.resetForm = function () {
        $scope.data.id = null;
        $scope.data.xpress = null;
        $scope.data.i = null;
        $scope.data.keyV = null;
        $scope.data.memo = null;
        $scope.data.weight = null;
        $scope.data.selecteExprIdx = null;
        $scope.data.keyVeg = null;
    };
    $scope.resetForm();
    $scope.getRefBank = function () {

        Ivkr.get(
                "/ksyt/referenz/?group=LIST_OF_BANK&scope=SYST_ADMNSTR",
                function (rsp) {
                    $scope.lsRefBank = rsp.body;
                    $scope.refBank.selected = $scope.lsRefBank[0];
                    $scope.getSlipType();
                },
                function (err) {
                    $log.info(err)
                }
        );
        /*$http.get(
         "/ksyt/referenz/?group=LIST_OF_BANK&scope=SYST_ADMNSTR",
         ).success(function (rsp) {
         $scope.lsRefBank = rsp.body;
         $scope.refBank.selected = $scope.lsRefBank[0];
         $scope.getSlipType();
         }).error(function (err) {
         $log.info(err);
         });*/
    };
    $scope.getRefBank();
    $scope.refSlipType = {};
    $scope.lsSlipType = [];
    $scope.getSlipType = function () {

        Ivkr.get(
                "/ksyt/referenz/?group=LIST_OF_SLIP_TYPE&scope=SYST_ADMNSTR",
                function (rsp) {
                    $scope.lsSlipType = rsp.body;
                    $scope.refSlipType.selected = $scope.lsSlipType[0];
                    $scope.getXprsn();
                },
                function (err) {
                    $log.info(err);
                }
        );
        /*$http.get(
         "/ksyt/referenz/?group=LIST_OF_SLIP_TYPE&scope=SYST_ADMNSTR",
         ).success(function (rsp) {
         $scope.lsSlipType = rsp.body;
         $scope.refSlipType.selected = $scope.lsSlipType[0];
         $scope.getXprsn();
         }).error(function (err) {
         $log.info(err);
         });*/
    };
    function onSuccess(bdy) {
        toaster.success({body: bdy});
    }

    $scope.lsXprsn = [];
    $scope.getXprsn = function () {

        Ivkr.get(
                "/ksyt/ccexpression/byBankAndNoteType?bank=" + $scope.refBank.selected.rname + "&noteType=" + $scope.refSlipType.selected.rname,
                function (rsp) {
                    $scope.lsXprsn = rsp.body;
                    $scope.data.checkedIndexExprsn = null;
                    $scope.getXprsnTestData("[" + $scope.refBank.selected.rname + "][" + $scope.refSlipType.selected.rname + "]");
                },
                function (err) {
                    $log.info(err);
                }
        );
        $scope.resetForm();
    };
    $scope.getXprsnTestData = function (name) {
        Ivkr.get(
                "/ksyt/referenz/?scope=SYST_ADMNSTR&group=XPRESSION_EVALUATION_TEST&name=" + name,
                function (rsp) {
                    if (rsp.body === null) {
                        Ivkr.get(
                                "/ksyt/referenz/?scope=SYST_ADMNSTR&group=XPRESSION_EVALUATION_TEST&name=[DEFAULT]",
                                function (rsp) {
                                    $log.info(rsp);
                                    $scope.data.memoEg = rsp.body;
                                },
                                function (err) {
                                    $log.info(err);
                                }
                        );
                    } else {
                        $scope.data.memoEg = rsp.body;
                    }

                },
                function (err) {
                    $log.info(err);
                }
        );
        $scope.resetForm();
    };
    $scope.saveXprsnTestData = function () {
        if ($scope.data.memoEg.rvalue !== '') {
            var id = $scope.data.memoEg.id;
            var name = $scope.data.memoEg.rname;
            var value = $scope.data.memoEg.rvalue.substring(0, 255);
            if ($scope.data.memoEg.rname === '[DEFAULT]') {
                id = null;
                name = "[" + $scope.refBank.selected.rname + "][" + $scope.refSlipType.selected.rname + "]";
            }
            Ivkr.post("/ksyt/referenz/", {
                "id": id,
                "gruppe": $scope.data.memoEg.gruppe,
                "rname": name,
                "rvalue": value,
                "scope": $scope.data.memoEg.scope
            }, function (rsp) {
                $scope.data.memoEg = rsp.body;
                toaster.success("Xprsn test data successfully saved " + name);
            }, function (err) {
                $log.info(err);
            });
        } else {
            toaster.warning("Xprsn test data is empty!");
        }

    };
    $scope.addExpr = function () {

        $http.post("/ksyt/ccexpression/validate", JSON.stringify({
            "xpress": $scope.data.xpress
        })).success(function (rsp) {

            if (rsp.body.xprEval === "00") {
                $scope.errMsg.xpr = '';
                var newLine = "\n";
                var i = 1;
                if ($scope.data.keyV === null || $scope.data.keyV === undefined || $scope.data.keyV === "") {
                    $scope.errMsg.xpr += i + ". Key-Value can not be empty." + newLine;
                    i++;
                }
                if ($scope.data.weight === null || $scope.data.weight === undefined || $scope.data.weight === "" || isNaN($scope.data.weight) || parseInt($scope.data.weight) === 0) {
                    $scope.errMsg.xpr += i + ". Weight can not be empty and must be a number." + newLine;
                    i++;
                }
                if ($scope.data.i === null || $scope.data.i === undefined || $scope.data.i === "" || isNaN($scope.data.i) || parseInt($scope.data.i) < 0) {
                    $scope.errMsg.xpr += i + ". i(Key-Value Order) can not be empty and must be a number and should greater than 0" + newLine;
                    i++;
                }
                if ($scope.data.memo === null || $scope.data.memo === undefined || $scope.data.memo === "") {
                    $scope.errMsg.xpr += i + ". Please provide a memo text." + newLine;
                    i++;
                }

                for (var itrXpr = 0; itrXpr < $scope.lsXprsn.length; itrXpr++) {
                    if ($scope.lsXprsn[itrXpr].id != $scope.data.id) {
                        if ($scope.lsXprsn[itrXpr].keyV.trim().toLowerCase() === $scope.data.keyV.trim().toLowerCase()) {
                            $scope.errMsg.xpr += i + ". Key-Value already exist" + newLine;
                            i++;
                            break;
                        }

                        if ($scope.lsXprsn[itrXpr].i == $scope.data.i) {
                            $scope.errMsg.xpr += i + ". Index already exist" + newLine;
                            i++;
                            break;
                        }
                    }
                }
                if ($scope.errMsg.xpr === '') {
                    if ($scope.data.selecteExprIdx === null) {
                        $scope.lsXprsn.push({
                            "id": $scope.data.id,
                            "xpress": $scope.data.xpress,
                            "i": $scope.data.i,
                            "keyV": $scope.data.keyV,
                            "bank": $scope.refBank.selected.rname,
                            "memo": $scope.data.memo,
                            "weight": $scope.data.weight,
                            "noteType": $scope.refSlipType.selected.rname
                        });
                        toaster.success("Xprn successfully add");
                        $scope.resetForm();
                    } else {
                        $scope.lsXprsn[$scope.data.selecteExprIdx].xpress = $scope.data.xpress;
                        $scope.lsXprsn[$scope.data.selecteExprIdx].i = $scope.data.i;
                        $scope.lsXprsn[$scope.data.selecteExprIdx].keyV = $scope.data.keyV;
                        $scope.lsXprsn[$scope.data.selecteExprIdx].memo = $scope.data.memo;
                        $scope.lsXprsn[$scope.data.selecteExprIdx].weight = $scope.data.weight;
                        toaster.success("Xprn successfully update");
                    }
                }

            } else {
                toaster.warning("Evaluation failed due to " + rsp.body.xprEval);
                $scope.errMsg.xpr = rsp.body.xprEval;
            }

        }).error(function (err) {
            $log.info(err);
            toaster.warning("Xprsn evaluation failed");
        });
    };
    $scope.deleteExpr = function () {
        if ($scope.data.selecteExprIdx === null || $scope.data.selecteExprIdx === '') {
            toaster.warning("Please select Xprsn.");
        } else {
            $scope.lsXprsn.splice($scope.data.selecteExprIdx, 1);
            toaster.warning("Successfully deleted of " + $scope.data.selecteExprIdx);
            $scope.resetForm();
        }

    };
    $scope.selectExpr = function (i) {
        $scope.data.selecteExprIdx = i;
        $scope.data.id = $scope.lsXprsn[i].id;
        $scope.data.xpress = $scope.lsXprsn[i].xpress;
        $scope.data.i = $scope.lsXprsn[i].i;
        $scope.data.keyV = $scope.lsXprsn[i].keyV;
        $scope.data.memo = $scope.lsXprsn[i].memo;
        $scope.data.weight = $scope.lsXprsn[i].weight;
    };
    $scope.data.keyVeg = null;
    $scope.setKeyVeg = function () {
        $scope.lsXprsn[$scope.data.selecteExprIdx].keyVeg = $scope.data.keyVeg;
        $scope.data.keyVeg = null;
        $scope.data.checkedIndexExprsn = null;
    };
    $scope.validateExpr = function () {

        Ivkr.post(
                "/ksyt/ccexpression/validate",
                {
                    "xpress": $scope.data.xpress
                },
                function (rsp) {
                    if (rsp.body.xprEval === "00") {
                        onSuccess("Xprsn evaluated succesfully");
                        $scope.errMsg.xpr = '';
                    } else {
                        toaster.warning("Evaluation failed due to " + rsp.body.xprEval);
                        $scope.errMsg.xpr = rsp.body.xprEval;
                    }
                },
                function (err) {
                    $log.info(err);
                    toaster.warning("Xprsn evaluation failed");
                }
        );
        /*$http.post("/ksyt/ccexpression/validate", JSON.stringify({
         "xpress": $scope.data.xpress
         })).success(function (rsp) {
         
         if (rsp.body.xprEval === "00") {
         onSuccess("Xprsn evaluated succesfully");
         $scope.errMsg.xpr = '';
         } else {
         onWarning("Evaluation failed due to " + rsp.body.xprEval);
         $scope.errMsg.xpr = rsp.body.xprEval;
         }
         
         }).error(function (err) {
         $log.info(err);
         onWarning("Xprsn evaluation failed");
         });*/
    };
    $scope.saveExprs = function () {
        Ivkr.post(
                "/ksyt/ccexpression/save/asrfnce?bank=" + $scope.refBank.selected.rname + "&noteType=" + $scope.refSlipType.selected.rname,
                $scope.lsXprsn,
                function (rsp) {
                    toaster.success("Save Xprsns succeed");
                    $scope.getXprsn();
                },
                function (err) {
                    $log.info(err);
                    toaster.warning("Save Xprsns failed");
                }
        );
        /*$http.post("/ksyt/ccexpression/save/asrfnce?bank=" + $scope.refBank.selected.rname + "&noteType=" + $scope.refSlipType.selected.rname,
         JSON.stringify($scope.lsXprsn)).success(function (rsp) {
         onSuccess("Save Xprsns succeed");
         $scope.getXprsn();
         }).error(function (err) {
         $log.info(err);
         onWarning("Save Xprsns failed");
         });*/
    };
    $scope.calcExprn = function () {
        for (var i = 0; i < $scope.lsXprsn.length; i++) {
            $scope.lsXprsn[i].tag = ".";
            callCalcExprn(i);
        }
    };
    function callCalcExprn(i) {
        var temp = $scope.lsXprsn[i].keyVeg;
        if (temp === undefined || temp === null)
            temp = '';
        Ivkr.post(
                "/ksyt/ccexpression/evaluate/asrfnce",
                {
                    "V": $scope.data.memoEg.rvalue,
                    "in": temp,
                    "xpress": $scope.lsXprsn[i].xpress
                },
                function (rsp) {
                    if (rsp.body) {
                        $scope.lsXprsn[i].tag = "FIT";
                    } else {
                        $scope.lsXprsn[i].tag = "DISF!";
                    }
                },
                function (err) {
                    $log.info(err);
                    onWarning(err);
                    $scope.lsXprsn[i].tag = "ERR!";
                }
        );
    }

}

//@010 
function rfrcDataCtrl($scope, $http, $log, toaster, Ivkr) {

    $scope.temp = {};
    $scope.state = {};
    $scope.reference = {};
    $scope.data = {};
    $scope.data.refId = null;
    $scope.data.refName = null;
    $scope.data.refValue = null;
    $scope.data.refScope = null;
    $scope.refInput = {};
    $scope.refInput.selected = {};
    $scope.lsRefInput = [];

    $scope.getRefInput = function () {
        Ivkr.get("/ksyt/referenz/?group=GENERIC_REFERNCE_TYPE&scope=SYST_OWNER", function (rsp) {
            $log.info(rsp);
            $scope.lsRefInput = rsp.body;
            $scope.refInput.selected = $scope.lsRefInput[0];
            $scope.getRef();
        }, function (err) {
            toaster.warning(err);
        });
    };
    $scope.getRefInput();
    $scope.lsRef = [];
    $scope.getRef = function (isQuerying) {

        var group = $scope.refInput.selected.rname;
        var qReq = "/ksyt/referenz/?group=" + group + "&scope=SYST_ADMNSTR";
        if (isQuerying)
            qReq = "/ksyt/referenz/?group=" + group + "&scope=SYST_ADMNSTR&q=" + $scope.data.refName;
        Ivkr.get(qReq, function (rsp) {
            $scope.lsRef = rsp.body;
            $scope.data.checkedIndexRefernz = null;
        }, function () {
            toaster.warning("failed load " + group);
        });
    };
    $scope.doResetForm = function () {
        $scope.data.refId = null;
        $scope.data.refName = null;
        $scope.data.refValue = null;
        $scope.data.refScope = null;
        $scope.getRef();
    };
    $scope.selectRef = function (index) {
        $scope.data.refId = $scope.lsRef[index].id;
        $scope.data.refName = $scope.lsRef[index].rname;
        $scope.data.refValue = $scope.lsRef[index].rvalue;
        $scope.data.refScope = $scope.lsRef[index].scope;
    };
    $scope.state.refValidity = true;
    $scope.doSave = function () {
        if ($scope.data.refName === null || $scope.data.refName === '' || $scope.refInput.selected === undefined) {
            $scope.refValidity = false;
        } else {
            $scope.refValidity = true;
            if ($scope.data.refScope === null || $scope.data.refScope === '') {
                $scope.data.refScope = "SYST_ADMNSTR";
            }

            Ivkr.post("/ksyt/referenz/", {
                "id": $scope.data.refId,
                "gruppe": $scope.refInput.selected.rname,
                "rname": $scope.data.refName,
                "rvalue": $scope.data.refValue,
                "scope": $scope.data.refScope
            }, function (rsp) {
                $scope.data.refId = rsp.body.id;
                toaster.success("Ref [" + rsp.body.id + "] saved succesfully");
                $scope.doResetForm();
            }, function (err) {
                $log.info(err);
            });
        }

    };
    $scope.doDelete = function () {
        $log.info($scope.refInput.selected);
        if ($scope.data.refId === null || $scope.data.refName === '') {
            $scope.refValidity = false;
        } else {
            $scope.refValidity = true;
            Ivkr.delete("/ksyt/referenz/" + $scope.data.refId, function () {}, function () {});
            $http.delete("/ksyt/referenz/" + $scope.data.refId).success(function (rsp) {
                $log.info(rsp);
                toaster.success("Ref [" + $scope.data.refId + "] deleted succesfully");
                $scope.doResetForm();
            }).error(function (err) {
                toaster.success("Ref deletion failed");
            });
        }

    };
}

//@010 
function rfrcSystDataCtrl($scope, $http, $log, toaster, Ivkr) {

    $scope.temp = {};
    $scope.state = {};
    $scope.reference = {};
    $scope.data = {};
    $scope.data.refId = null;
    $scope.data.refName = null;
    $scope.data.refValue = null;
    $scope.data.refScope = null;
    $scope.data.refGruppe = null;
    $scope.refInput = {};
    $scope.refInput.selected = {};
    $scope.lsRefInput = [];

    $scope.getRefInput = function () {
        Ivkr.get("/ksyt/referenz/scopegruppe/syst", function (rsp) {
            $log.info(rsp);
            $scope.lsRefInput = [];
            for (var i = 0; i < rsp.body.length; i++) {
                $scope.lsRefInput.push({
                    "gruppe": rsp.body[i][0],
                    "scope": rsp.body[i][1],
                    "rname": rsp.body[i][0],
                    "rvalue": rsp.body[i][0]
                });
            }
            $scope.refInput.selected = $scope.lsRefInput[0];
            $scope.getRef();
        }, function (err) {
            toaster.warning(err);
        });
    };
    $scope.getRefInput();
    $scope.lsRef = [];
    $scope.getRef = function (isQuerying) {

        var group = $scope.refInput.selected.gruppe;
        var scope = $scope.refInput.selected.scope;
        var qReq = "/ksyt/referenz/?group=" + group + "&scope=" + scope;
        if (isQuerying)
            qReq = "/ksyt/referenz/?group=" + group + "&scope=" + scope + "&q=" + $scope.data.refName;
        Ivkr.get(qReq, function (rsp) {
            $scope.lsRef = rsp.body;
            $scope.data.checkedIndexRefernz = null;
        }, function () {
            toaster.warning("failed load " + group);
        });
    };
    $scope.doResetForm = function () {
        $scope.data.refId = null;
        $scope.data.refName = null;
        $scope.data.refValue = null;
        $scope.data.refScope = null;
        $scope.data.refGruppe = null;
        $scope.getRef();
    };
    $scope.selectRef = function (index) {
        $scope.data.refId = $scope.lsRef[index].id;
        $scope.data.refName = $scope.lsRef[index].rname;
        $scope.data.refValue = $scope.lsRef[index].rvalue;
        $scope.data.refScope = $scope.lsRef[index].scope;
        $scope.data.refGruppe = $scope.lsRef[index].gruppe;
    };
    $scope.state.refValidity = true;
    $scope.doSave = function () {
        if ($scope.data.refName === null || $scope.data.refName === '' || $scope.refInput.selected === undefined) {
            $scope.refValidity = false;
        } else {
            $scope.refValidity = true;

            var refScope = $scope.data.refScope;
            if ($scope.data.refScope === null) {
                refScope = $scope.refInput.selected.scope;
            }

            var refGruppe = $scope.data.refGruppe;
            if ($scope.data.refGruppe === null) {
                refGruppe = $scope.refInput.selected.gruppe;
            }

            Ivkr.post("/ksyt/referenz/", {
                "id": $scope.data.refId,
                "gruppe": refGruppe,
                "rname": $scope.data.refName,
                "rvalue": $scope.data.refValue,
                "scope": refScope
            }, function (rsp) {
                $scope.data.refId = rsp.body.id;
                toaster.success("Ref [" + rsp.body.id + "] saved succesfully");
                $scope.doResetForm();
            }, function (err) {
                $log.info(err);
            });
        }

    };
    $scope.doDelete = function () {
        $log.info($scope.refInput.selected);
        if ($scope.data.refId === null || $scope.data.refName === '') {
            $scope.refValidity = false;
        } else {
            $scope.refValidity = true;
            Ivkr.delete("/ksyt/referenz/" + $scope.data.refId, function () {}, function () {});
            $http.delete("/ksyt/referenz/" + $scope.data.refId).success(function (rsp) {
                $log.info(rsp);
                toaster.success("Ref [" + $scope.data.refId + "] deleted succesfully");
                $scope.doResetForm();
            }).error(function (err) {
                toaster.success("Ref deletion failed");
            });
        }

    };
    $scope.doSync = function () {
        Ivkr.post("/ksyt/referenz/sync", {}, function () {
            toaster.success("Sync success");
        }, function () {
            toaster.warning("Sync failed!");
        });
    }
}


//@008
function mutasiInputRawsCtrl(Ivkr, $rootScope, $scope, $http, $log, toaster) {

    $scope.temp = {};
    $scope.state = {};
    $scope.data = {};
    $scope.state.savingIteration = 0;
    $scope.state.onSavingProcess = false;
    $scope.state.saved = false;
    $scope.state.uploadMutasi = true;
    $scope.data.submission = {};
    $scope.data.submission.accessfile = undefined;
    /*$http.get("/ksyt/xcross/generate?file=&type=.mmst")
     .then(function (response) {
     $scope.temp.lmutasi = response.data.body;
     });*/

    $scope.uploadFile = function (files) {
        $scope.state.saved = false;
        $scope.state.itrSaving = 0;
        $scope.data.submission = {};
        $scope.data.submission.fileName = files[0].name;
        Ivkr.upload("/ksyt/xcross/upload?fileName=" + $scope.data.submission.fileName,
                files[0], {transformRequest: angular.identity, headers: {'Content-Type': undefined}},
                function (rsp) {
                    $scope.temp.lmutasi = rsp.body;
                    toaster.success("Successfully upload: " + $scope.data.submission.fileName + ", " + $scope.temp.lmutasi.content.length + " rows");
                    $scope.state.uploadMutasi = true;
                    $scope.state.errMsg = undefined;
                },
                function (err, code) {
                    $log.info("err!" + code);
                    $log.info(err);
                    $scope.state.uploadMutasi = false;
                    $scope.state.errMsg = err.header.status + ":" + err.header.code + " " + err.header.info;
                    toaster.warning("Upload " + $scope.data.submission.fileName + " failed");

                }

        );
    };
    $scope.submit = function () {
        $scope.data.submission.submitSystDate = new Date();
        $scope.state.onSavingProcess = true;
        saveSubmision();
    };
    /*$scope.generate = function () {
     $http.get("/ksyt/mutasi/generate/mmst?sbssnId=" + $scope.data.submission.id
     ).error(function (err) {
     $log.info(err);
     });
     };*/

    $scope.genAccess = function (id, fn) {
        $log.info("genAccess=" + id);
        Ivkr.get("/ksyt/mutasi/file-access/" + id,
                function (rsp) {
                    $scope.data.submission.accessfile = rsp.body;
                    $log.info("accessfile:" + $scope.data.submission.accessfile);
                    fn();
                },
                function (err) {
                    fn();
                }
        );
    }
    ;
    // Compose Mandiri Mutasi Date
    function cmmd(date) {
        var dd = date.split("/")[0];
        var mm = date.split("/")[1];
        var yyyy = date.split("/")[2];
        return yyyy
                + "-" + (mm.length < 2 ? "0" + mm : mm)
                + "-" + (dd.length < 2 ? "0" + dd : dd);
    }

    // Normalized Mandiri Mutasi Number
    function nmmn(n) {
        var ds = n.charAt(n.length - 3);
        if (ds === '.')
            return parseFloat(n.replace(/\,/g, '').replace(/\./g, ","));
        else if (ds === ',')
            return parseFloat(n.replace(/\./g, ''));
        else
            return n;
    }

    function getNum(text) {
        return text.replace(/\D+/g, '');
    }

    $scope.renderStatusLabel = function (status) {
        var label = "label ";
        if (status === "saved") {
            label += "label-success ";
        } else if (status === "FAILED") {
            label += "label-warning ";
        } else {
            label += "label-default ";
        }
        return label + "pull-right"
    }

    function saveMutasiAt(i, sId) {


        Ivkr.post("/ksyt/mutasi",
                {
                    "sbssnId": sId,
                    "tgl": cmmd($scope.temp.lmutasi.content[i].Tanggal),
                    "kredit": nmmn($scope.temp.lmutasi.content[i].Kredit),
                    "debit": nmmn($scope.temp.lmutasi.content[i].Debet),
                    "rekAccount": getNum($scope.temp.lmutasi.headers.NomorRekening),
                    "memo": $scope.temp.lmutasi.content[i].KeteranganTransaksi,
                    "stage": 5.0,
                    "actor": $rootScope.activeUser.uName,
                    "systDate": $scope.data.submission.submitSystDate,
                    "systMemo": "@" + i + "|",
                    "incomeId": null
                },
                function (rsp) {
                    $scope.state.itrSaving++;
                    // saved or exist
                    angular.extend($scope.temp.lmutasi.content[i], {status: 'saved', id: rsp.body.id});
                    //$log.info("$scope.state.itrSaving:" + $scope.state.itrSaving);
                    if ($scope.state.onSavingProcess && $scope.temp.lmutasi.content.length <= $scope.state.itrSaving) {

                        toaster.warning("Processing of '" + $scope.data.submission.fileName + "' end!");
                        $scope.state.onSavingProcess = false;
                        $scope.state.saved = true;
                        $scope.state.itrSaving = 0;
                    }
                },
                function (err) {
                    $scope.state.itrSaving++;
                    angular.extend($scope.temp.lmutasi.content[i], {status: err.header.code});
                    //$log.info("$scope.state.itrSaving:" + $scope.state.itrSaving);
                    if ($scope.state.onSavingProcess && $scope.temp.lmutasi.content.length <= $scope.state.itrSaving) {

                        toaster.warning("Processing of '" + $scope.data.submission.fileName + "' end!");
                        $scope.state.onSavingProcess = false;
                        $scope.state.saved = true;
                        $scope.state.itrSaving = 0;
                    }
                }
        );
    }
    function saveSubmision() {

        Ivkr.post("/ksyt/mutasisubmission/",
                {
                    "headers": JSON.stringify($scope.temp.lmutasi.headers),
                    "fileSrc": $scope.data.submission.fileName,
                    "nrow": $scope.temp.lmutasi.content.length,
                    "content": JSON.stringify($scope.temp.lmutasi.content),
                    "memo": "",
                    "actor": $rootScope.activeUser.uName,
                    "systDate": $scope.data.submission.submitSystDate
                },
                function (rsp) {

                    $scope.data.submission.id = rsp.body.id;
                    toaster.success("Submission of " + $scope.data.submission.fileName + " success with id=" + $scope.data.submission.id);
                    $scope.genAccess($scope.data.submission.id, function () {
                        for (var i = 0; i < $scope.temp.lmutasi.content.length; i++) {
                            if (!$scope.temp.lmutasi.content[i].status || $scope.temp.lmutasi.content[i].status === "FAILED") {
                                saveMutasiAt(i, $scope.data.submission.id);
                            }
                        }
                    });
                },
                function (err, code) {
                    $log.info("err!" + code);
                    $log.info(err);
                    toaster.warning("Submission failure of " + $scope.data.submission.fileName);
                    $scope.state.errMsg = err.header.status + "[" + err.header.code + "]: " + err.header.info;
                    $scope.state.onSavingProcess = false;
                });
    }

}

//@007 #CURR
function mutasiInputIncomeCtrl(Ivkr, $rootScope, $state, $scope, $log, toaster, SweetAlert) {

    $scope.today = new Date();
    $scope.state = {};
    $scope.state.isOnHunt = false;
    $scope.lsHookDetail = [];
    $scope.lsCMutasi = [];
    $scope.data = {};
    $scope.data.date = new moment();
    $scope.data.time = new moment().format('HH:MM');


    $scope.normDate = function (vdate) {
        return vdate.format('YYYY-MM-DD');
    };

    $scope.account = {};
    $scope.lsAccount = [];
    $scope.getRefAccount = function () {
        Ivkr.get(
                "/ksyt/referenz/?group=LIST_OF_REKENING_SCC&scope=SYST_ADMNSTR",
                function (rsp) {
                    $scope.lsAccount = [];
                    for (var i = 0; i < rsp.body.length; i++) {
                        $scope.lsAccount.push({text: rsp.body[i].rvalue, accountNo: rsp.body[i].rname});
                    }
                },
                function (err) {
                    $log.info(err);
                }
        );
    };
    $scope.incomeType = {};
    $scope.lsIncomeType = [];
    $scope.getRefIncomeType = function () {
        Ivkr.get(
                "/ksyt/referenz/?group=LIST_OF_INCOME_TYPE&scope=SYST_ADMNSTR",
                function (rsp) {
                    $scope.lsIncomeType = [];
                    for (var i = 0; i < rsp.body.length; i++) {
                        $scope.lsIncomeType.push({desc: rsp.body[i].rvalue, text: rsp.body[i].rname});
                    }
                    $scope.getRefAccount();
                },
                function (err) {
                    $log.info(err);
                }
        );
    };
    $scope.noteType = {};
    $scope.lsNoteType = [];
    $scope.getRefSlipType = function () {
        Ivkr.get(
                "/ksyt/referenz/?group=LIST_OF_SLIP_TYPE&scope=SYST_ADMNSTR",
                function (rsp) {
                    $scope.lsNoteType = [];
                    for (var i = 0; i < rsp.body.length; i++) {
                        $scope.lsNoteType.push({desc: rsp.body[i].rvalue, text: rsp.body[i].rname});
                    }
                    $scope.getRefIncomeType();
                },
                function (err) {
                    $log.info(err);
                }
        );
    };
    $scope.bank = {};
    $scope.lsBank = [];
    $scope.getRefBank = function () {
        Ivkr.get(
                "/ksyt/referenz/?group=LIST_OF_BANK&scope=SYST_ADMNSTR",
                function (rsp) {
                    $scope.lsBank = [];
                    for (var i = 0; i < rsp.body.length; i++) {
                        $scope.lsBank.push({desc: rsp.body[i].rvalue, text: rsp.body[i].rname});
                    }
                    $scope.getRefSlipType();
                },
                function (err) {
                    $log.info(err);
                }
        );
    };
    $scope.getRefBank();
    function postIXDetail(i, payload) {
        Ivkr.post(
                "/ksyt/incomexdetail/",
                payload,
                function (rsp) {
                    $log.info(rsp);
                    $scope.lsHookDetail[i].incomexdetailId = rsp.body.incomexdetailId;
                    toaster.success("Detail data[" + $scope.lsHookDetail[i].incomexdetailId + "] inserted succesfully");
                },
                function (err) {
                    $log.info(err);
                }
        );
    }

    $scope.errMsg = {};
    $scope.errMsg.formPemasukan = "";
    function validateInput() {
        $scope.errMsg.formPemasukan = "";
        var newLine = "\n";
        var i = 1;
        if ($scope.settedCroppedImage === "") {
            $scope.errMsg.formPemasukan = $scope.errMsg.formPemasukan + i + "). " + "Kapture Digital harus dalam mode SET!" + newLine;
            i++;
        }
        $log.info($scope.data.date);
        if ($scope.data.date === undefined || $scope.data.date === "") {
            $scope.errMsg.formPemasukan = $scope.errMsg.formPemasukan + i + "). " + "Tgl Transaksi harus terisi" + newLine;
            i++;
        }
        $log.info($scope.data.time);
        if ($scope.data.time === undefined || $scope.data.time === "") {
            $scope.errMsg.formPemasukan = $scope.errMsg.formPemasukan + i + "). " + "Waktu Transaksi harus terisi" + newLine;
            i++;
        }
        if ($scope.data.nominal === undefined || $scope.data.nominal === "" || isNaN($scope.data.nominal) || parseInt($scope.data.nominal) === 0) {
            $scope.errMsg.formPemasukan = $scope.errMsg.formPemasukan + i + "). " + "Nominal bernilai angka > 0 dan harus terisi" + newLine;
            i++;
        }
        if ($scope.data.memo === undefined || $scope.data.memo === "") {
            $scope.errMsg.formPemasukan = $scope.errMsg.formPemasukan + i + "). " + "Memo harus terisi" + newLine;
            i++;
        }
        if ($scope.bank.selected === undefined || $scope.bank.selected === "") {
            $scope.errMsg.formPemasukan = $scope.errMsg.formPemasukan + i + "). " + "Bank pengirim harus terisi" + newLine;
            i++;
        }
        if ($scope.account.selected === undefined || $scope.account.selected === "") {
            $scope.errMsg.formPemasukan = $scope.errMsg.formPemasukan + i + "). " + "Akun Bank SCC harus terisi" + newLine;
            i++;
        }
        if ($scope.noteType.selected === undefined || $scope.noteType.selected === "") {
            $scope.errMsg.formPemasukan = $scope.errMsg.formPemasukan + i + "). " + "Jenis Bukti Transfer harus terisi" + newLine;
            i++;
        }

        for (var j = 0; j < $scope.lsHookDetail.length; j++) {
            if ($scope.lsHookDetail[j].V === undefined || $scope.lsHookDetail[j].V === "") {
                $scope.errMsg.formPemasukan = $scope.errMsg.formPemasukan + i + "). " + "Detail[" + $scope.lsHookDetail[j].keyV + "] Transfer harus terisi" + newLine;
                i++;
            }
        }
        /**
         *         
         SweetAlert.swal({
         title: "Penyimpanan Data",
         text: "Data-pemasukan akan tersimpan sebagai draft pada tahap ini",
         type: "warning",
         showCancelButton: true,
         confirmButtonColor: "#DD6B55",
         confirmButtonText: "Ya, draft data ini!",
         cancelButtonText: "Batalkan",
         closeOnConfirm: false,
         closeOnCancel: false},
         function (isConfirm) {
         if (isConfirm) {
         SweetAlert.swal("Tersimpan!", "Telah tersimpan sebagai draft!", "success");
         return $scope.errMsg.formPemasukan === "";
         } else {
         SweetAlert.swal("Cancelled", "Your imaginary file is safe :)", "error");
         $scope.state.isOnHunt = true;
         return false;
         }
         });
         
         */
        return $scope.errMsg.formPemasukan === "";
    }
    $scope.settedCroppedImageFSL = null;
    $scope.data.selectedDate = null;
    $scope.data.incomeId = null;
    function keepIncome() {
        if ($scope.data.incomeId !== undefined && $scope.data.incomeId !== null && $scope.data.incomeId !== '') {
            toaster.warning("$scope.data.incomeId=" + $scope.data.incomeId);
        } else {
            toaster.success("$scope.data.incomeId=" + $scope.data.incomeId);
        }
        $log.info($scope.data.date);
        $log.info("formatedDate" + $scope.data.date.format('YYYY-MM-DD'));
        $scope.data.selectedDate = new moment().format('YYYY-MM-DD');
        if ($scope.data.date !== undefined) {
            $scope.data.selectedDate = $scope.data.date.format('YYYY-MM-DD');
        }
        var payloadIncome = {
            "id": $scope.data.incomeId,
            "trxDate": $scope.data.selectedDate,
            "trxTime": $scope.data.time + ":00",
            "nominal": $scope.data.nominal,
            "printSrc": $scope.settedCroppedImage,
            "printFSL": $scope.settedCroppedImageFSL,
            "memo": $scope.data.memo,
            "rekAccount": $scope.account.selected.accountNo,
            "bank": $scope.bank.selected.text,
            "noteType": $scope.noteType.selected.text,
            "type": $scope.incomeType.selected.text,
            "stage": 5.0,
            "actor": $rootScope.activeUser.uName,
            "systDate": new Date(),
            "systMemo": "Kimosyst-Submission|",
            "mutasiId": null,
            "tweight": 0
        };
        Ivkr.post(
                "/ksyt/income",
                payloadIncome,
                function (rsp) {
                    $scope.data.incomeId = rsp.body.incomeId;
                    $scope.settedCroppedImageFSL = rsp.body.fsl;
                    $log.info("incomeId=" + $scope.data.incomeId);
                    toaster.success("[" + $scope.data.incomeId + "]Income data inserted successfully=" + $scope.settedCroppedImageFSL);
                    for (var i = 0; i < $scope.lsHookDetail.length; i++) {
                        $log.info("incomexdetailId[" + i + "]=" + $scope.lsHookDetail[i].incomexdetailId);
                        postIXDetail(i, {
                            "id": $scope.lsHookDetail[i].incomexdetailId,
                            "incomeId": $scope.data.incomeId,
                            "xpressId": $scope.lsHookDetail[i].id,
                            "pname": $scope.lsHookDetail[i].keyV,
                            "pvalue": $scope.lsHookDetail[i].V,
                            "systDate": new Date(),
                            "systMemo": $scope.lsHookDetail[i].xpress + "|" + $scope.lsHookDetail[i].weight + "|"
                        });
                    }

                },
                function (err) {
                    $log.error(err);
                }
        );
    }

    $scope.verifiedReady = false;
    $scope.freezeAll = function () {
        if ($scope.state.isOnHunt) {
            if (validateInput()) {
                keepIncome();
                $scope.retrieveCluster();
                $scope.verifiedReady = true;
            } else {
                $scope.verifiedReady = false;
            }
        }
    };
    var nCalculateWeight = 0;
    $scope.calculateWeight = function () {
        nCalculateWeight = $scope.lsCMutasi.length;
        $scope.data.selectedIndexCMutasi = -1;
        toaster.success("Calculate weight starting");
        for (var i = 0; i < $scope.lsCMutasi.length; i++) {
            callWCalc(i, $scope.data.incomeId, $scope.lsCMutasi[i].id);
        }
    };
    /*   
     var list = {"you": 100, "me": 75, "foo": 116, "bar": 15};
     keysSorted = Object.keys(list).sort(function(a,b){return list[a]-list[b]})
     alert(keysSorted);
     */

    function compareWeight(mutasi0, mutasi1) {
        if (mutasi0.weight > mutasi1.weight)
            return -1;
        if (mutasi0.weight < mutasi1.weight)
            return 1;
        return 0;
    }


    function callWCalc(i, incomeId, mutasiId) {
        Ivkr.get(
                "/ksyt/income/wcalc/" + incomeId + "/" + mutasiId,
                function (rsp) {
                    $scope.lsCMutasi[i].weight = rsp.body;
                    nCalculateWeight--;
                    if (nCalculateWeight === 0) {
                        $scope.lsCMutasi.sort(compareWeight);
                        $scope.data.selectedIndexCMutasi = null;
                        toaster.warning("Calculate weight end");
                    }
                },
                function (err) {
                    $log.info(err);
                }
        );
    }

    $scope.data.selectedIndexCMutasi = -1;
    $scope.data.incomeCMutasiId = null;
    $scope.data.incomeCMutasiFSL = null;
    $scope.storePair = function () {
        toaster.pop("Telah dipilih dengan index " + $scope.data.selectedIndexCMutasi);
        $log.info($scope.lsCMutasi[$scope.data.selectedIndexCMutasi]);
        if ($scope.data.selectedIndexCMutasi !== -1 && $scope.lsCMutasi[$scope.data.selectedIndexCMutasi].id !== undefined) {
            comply($scope.data.selectedIndexCMutasi);
        } else {
            toaster.warning("No mutasi data selected!");
        }
    };
    function comply(i) {
        if ($scope.data.incomeCMutasiId === null) {
            Ivkr.post(
                    "/ksyt/incomecmutasi/comply",
                    {
                        "id": $scope.data.incomeCMutasiId,
                        "incomeId": $scope.data.incomeId,
                        "mutasiId": $scope.lsCMutasi[i].id,
                        "actor": $rootScope.activeUser.uName,
                        "memo": null,
                        "weight": $scope.nvl($scope.lsCMutasi[i].weight, -1),
                        "systDate": new Date(),
                        "stage": 9.9
                    },
                    function (rsp) {
                        $log.info(rsp);
                        $scope.data.incomeCMutasiId = rsp.body.incomeCMutasiId;
                        toaster.success("$scope.data.incomeCMutasiId=" + $scope.data.incomeCMutasiId);
                        complyImg();
                    },
                    function (err) {
                        $log.info(err);
                    }
            );
        } else {
            onWarning("Complience is already saved!");
        }


    }

    $scope.state.complied = false;
    function complyImg() {
        if ($scope.data.incomeCMutasiId !== null) {

            Ivkr.post(
                    "/ksyt/incomecmutasi/complied/img/" + $scope.data.incomeCMutasiId, "",
                    function (rsp) {
                        $scope.data.incomeCMutasiFSL = rsp.body.imgPblcURI;
                        $log.info(rsp);
                        toaster.success("Compliencd image has been created, " + $scope.data.incomeCMutasiFSL);
                        $scope.state.complied = true;
                        $scope.verifiedReady = false;
                        var tMutasi = $scope.lsCMutasi[$scope.data.selectedIndexCMutasi];
                        tMutasi.stage = 9.9;
                        $scope.lsCMutasi = [];
                        $scope.lsCMutasi[0] = tMutasi;
                        $log.info($scope.lsCMutasi[0]);
                    },
                    function (err) {
                        $log.info(err);
                    }
            );
        } else {
            toaster.warning("Compliencd image can not be created! please save complience data first!");
        }
    }

    $scope.state.clusterType = 'lite';
    $scope.retrieveCluster = function () {
        $log.info("clusterType:" + $scope.state.clusterType + "[" + $scope.data.selectedDate + "," + $scope.account.selected.accountNo + "," + $scope.data.nominal + "]");
        $scope.lsCMutasi = [];
        $scope.data.selectedIndexCMutasi = -1;
        switch ($scope.state.clusterType) {
            case 'lite':
                clusterLite();
                break;
            case 'rete':
                clusterRete();
                break;
            case 'enti':
                clusterEnti();
                break;
        }
    };
    function clusterLite() {
        Ivkr.get(
                "/ksyt/mutasi/cluster/1_0?tgl=" + $scope.data.selectedDate + "&rekAccount=" + $scope.account.selected.accountNo + "&kredit=" + $scope.data.nominal,
                function (rsp) {
                    $scope.lsCMutasi = rsp.body;
                    toaster.success("Cluster Lite telah diload, kode:1_0");
                },
                function (err) {
                    toaster.warning("Cluster Lite gagal diload, kode:1_0");
                }
        );
    }

    function clusterRete() {
        Ivkr.get(
                "/ksyt/mutasi/cluster/2_0?tgl=" + $scope.data.selectedDate + "&rekAccount=" + $scope.account.selected.accountNo + "&kredit=" + $scope.data.nominal,
                function (rsp) {
                    $scope.lsCMutasi = rsp.body;
                    toaster.success("Cluster Rete telah diload, kode:2_1");
                },
                function (err) {
                    toaster.warning("Cluster Rete gagal diload, kode:2_1");
                }
        );
    }

    function clusterEnti() {
        Ivkr.get(
                "/ksyt/mutasi/cluster/2_1?tgl=" + $scope.data.selectedDate + "&rekAccount=" + $scope.account.selected.accountNo + "&kredit=" + $scope.data.nominal,
                function (rsp) {
                    $scope.lsCMutasi = rsp.body;
                    toaster.success("Cluster Enti telah diload, kode:2_1");
                },
                function (err) {
                    toaster.warning("Cluster Enti gagal diload, kode:2_1");
                }
        );
    }

    $scope.parseStage = function (stage) {
        switch (stage) {
            case 0 :
                return "NEW";
            case 9.9:
                return "OWNED";
        }
    };
    $scope.nvl = function (v, k) {
        if (v === undefined || v === null) {
            return k;
        } else {
            return v;
        }
    }

    $scope.rendHookDetail = function () {
        Ivkr.get(
                "/ksyt/ccexpression/byBankAndNoteType?bank=" + $scope.bank.selected.text + "&noteType=" + $scope.noteType.selected.text,
                function (rsp) {
                    $scope.lsHookDetail = rsp.body;
                },
                function (err) {
                    $log.info(err);
                }
        );
    };
    /**/
    $scope.dupa = "dasdasdas";
    $scope.isOnCropMode = true;
    $scope.settedCroppedImage = "";
    $scope.toggleCropMode = function () {
        $scope.isOnCropMode = !$scope.isOnCropMode;
        $scope.settedCroppedImage = $scope.tCroppedImage;
    };
//$scope.tImage = 'data:image/jpeg;base64,';
    $scope.tImage = '';
    $scope.tCroppedImage = '';
    var handleFileSelector = function (evt) {
        var file = evt.currentTarget.files[0];
        var reader = new FileReader();
        var tImg;
        reader.onload = function (evt) {
            $scope.$apply(function ($scope) {

                tImg = evt.target.result;
                $scope.tImage = tImg;
                //angular.copy(evt.target.result, $scope.myImage);
            });
        };
        reader.readAsDataURL(file);
    };
    angular.element(document.querySelector('#fileInputs')).on('change', handleFileSelector);

    $scope.reinit = function () {
        $state.reload();
    };


}



/**
 *
 * Pass all functions into module
 */
angular
        .module('inspinia')
        .controller('MainCtrl', MainCtrl)
        .controller('loginCtrl', loginCtrl)
        .controller('ReAuthorizationCtrl', ReAuthorizationCtrl)
        .controller('ConfirmationDialogCtrl', ConfirmationDialogCtrl)
        .controller('mutasiInputRawsCtrl', mutasiInputRawsCtrl)
        .controller('mutasiInputIncomeCtrl', mutasiInputIncomeCtrl)
        .controller('rfrcSystDataCtrl', rfrcSystDataCtrl)
        .controller('rfrcDataCtrl', rfrcDataCtrl)
        .controller('rfrcXPressionCtrl', rfrcXPressionCtrl)
        .controller('vwMutasiIncomeCompliance', vwMutasiIncomeCompliance)
        .controller('vwMutasiFindSubmission', vwMutasiFindSubmission)
        .controller('vwMutasiSubmission', vwMutasiSubmission)
        .controller('vwIncomeFindAllCtrl', vwIncomeFindAllCtrl);

